...
RSpec.describe GuestLimits do
  ...
  describe ".monthly_count_for" do
    let(:project) { Project.create!(name: "Guest Project") }

    it "returns 0 for a project with no optimizations" do
      expect(described_class.monthly_count_for(project.token)).to eq(0)
    end

    it "returns 0 for an unknown token" do
      expect(described_class.monthly_count_for("nonexistent")).to eq(0)
    end

    it "does not count the initial optimization for a project created this month" do
    ...
    it "counts additional optimizations beyond the first one" do
      ...
      expect(described_class.monthly_count_for(project.token)).to eq(4)
    end

    it "reaches the free plan limit at 10 additional optimizations" do
      max = Plannable::PLANS["free"][:max_monthly_optimizations_per_project]
      (max + 1).times { project.optimizations.create!(status: "completed", result: {}) }
      expect(described_class.monthly_count_for(project.token)).to eq(max)
    end

    it "does not count optimizations from previous months" do
